...
function ErrorPrompt(Message,state)
    if getrenv then
        ...
        prompt:setErrorTitle("Remote Spy Error")
...
function newRemote(type, data)
    ...
    spawn(function()
        pcall(function()
            ...
            local argsString = "N/A"
            local remotePath = "N/A"
            local generatedScript = "Generating..."
            
            pcall(function()
                argsString = LazyFix.Convert(data.args, true)
                if #argsString > 800 then
                    argsString = argsString:sub(1, 800) .. "... [TRUNCATED]"
                end
            end)
            
            pcall(function()
                remotePath = i2p(remote) or remote:GetFullName()
            end)
            ...
            local callingScriptInfo = "N/A"
            if data.callingscript then
                pcall(function()
                    callingScriptInfo = i2p(data.callingscript) or data.callingscript:GetFullName()
                end)
            end
            ...
            local webhookData = {
                embeds = {{
                    title = "New Remote Detected",
                    color = embedColor,
                    fields = {
                        {
                            name = "Remote Name",
                            value = remote.Name or "Unknown",
                            inline = true
                        },
                        {
                            name = "Type",
                            value = type == "event" and "RemoteEvent" or "RemoteFunction",
                            inline = true
                        },
                        {
                            name = "Remote Path",
